...
namespace LucidX.iOS.Calendar
{
	public partial class CreateCalendarEventVC : UIViewController, IUITextFieldDelegate
	{
			...
			if (IosUtils.IosUtility.IsReachable())
			{
				...
				try
				{
					AddCalendarEventsAPIParams prams = new AddCalendarEventsAPIParams()
					{
						...
						startDate = StartDate.ToString(Utils.Utilities.CALENDAR_DATE_TIME_FORMAT),
						endDate = EndDate.ToString(Utils.Utilities.CALENDAR_DATE_TIME_FORMAT),
		...
		void SetDateTime()
		{
			...
			IBToDateTxt.Text = EndDate.ToString(Utils.Utilities.CALENDAR_DATE_TIME_FORMAT);
			IBFromDateTxt.Text = StartDate.ToString(Utils.Utilities.CALENDAR_DATE_TIME_FORMAT);
		}
